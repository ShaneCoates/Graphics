...
void main()
{
	...
	vec3 ro = m_eye;
	vec3 rd = normalize(m_camForward * m_focalLength + m_camRight * uv.x * m_aspectRatio + m_camUp * uv.y);

	//vec4 color = computeColor(ro, rd);

	// 4xAA
	vec3 rd0 = normalize(m_camForward * m_focalLength + m_camRight * (uv.x - hps.x) * m_aspectRatio + m_camUp * uv.y);
	vec3 rd1 = normalize(m_camForward * m_focalLength + m_camRight * (uv.x + hps.x) * m_aspectRatio + m_camUp * uv.y);
	vec3 rd2 = normalize(m_camForward * m_focalLength + m_camRight * uv.x * m_aspectRatio + m_camUp * (uv.y - hps.y));
	vec3 rd3 = normalize(m_camForward * m_focalLength + m_camRight * uv.x * m_aspectRatio + m_camUp * (uv.y + hps.y));
